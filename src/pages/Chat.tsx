    ...
    return (
        <div className="flex flex-col h-screen">
            {chatMessages.map((message) => (
                 <div key={message.messageId} className="flex-1 flex flex-col space-y-4 overflow-y-auto pb-4">
                 <div className="flex flex-col items-end pr-6">
                     <p className="bg-[#303030] rounded-full p-2.5 max-w-[30%] w-fit">{message.prompt}</p>
                 </div>
                 <div className="flex flex-row justify-start pl-6 gap-5">
                     ...
                     <p className="max-w-[50%] w-fit">{message.response}</p>
                 </div>
             </div>
            ))}
